...
// 製品名 → 正規化したキャッシュキー
// OCR(Vision)は非決定的で、同じ写真でも補足ラベルや容量がブレて別名を出す
// （例: 「…（第3類医薬品）500錠」と「…（ビタミンC主薬製剤）500錠」）。
// そのままだとキーが不安定でキャッシュがヒットしないため、製品を一意に決める核
// （ブランド・剤形・メーカー・成分量mg）は残し、ブレやすいノイズだけを落とす。
function normalizeName(name) {
  let s = String(name || '').normalize('NFKC'); // 全角→半角・互換文字統一（（）→() 第３→第3 等）

  // 1) 規制区分・製剤区分などの補足ラベルを除去する。
  //    括弧内にこれらの語を含むときだけ括弧ごと削るので、ブランド括弧（例: バファリン(A)）は残る。
  const LABEL = '第\\d*類医薬品|主薬製剤|医薬部外品|機能性表示食品|栄養機能食品|特定保健用食品|特保|トクホ|健康食品';
  s = s.replace(new RegExp(`[(\\[【][^)\\]】]*(?:${LABEL})[^)\\]】]*[)\\]】]`, 'g'), ' ');
  //    括弧が無くても、ブランド語と紛れない明確なラベルだけは単独でも除去（主薬製剤は括弧時のみ）。
  s = s.replace(/指定第\d*類医薬品|第\d*類医薬品|医薬部外品|機能性表示食品|栄養機能食品|特定保健用食品/g, ' ');

  // 2) 容量・入数（パックサイズ）を除去。例: 500錠 / 30カプセル / 90粒 / 14包。
  //    成分量(mg/g/ml)や数値ブランド(例: ビタミンC2000)は識別に必要なので残す。
  s = s.replace(/\d+\s*(?:錠|カプセル|caps?|粒|包|袋|枚|本|個|回分|日分|食分)/gi, ' ');
